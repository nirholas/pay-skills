...
name: three-ws
title: "three.ws"
description: "Solana agent identity and 3D avatar API: glTF/GLB validation, Solana token-to-mesh, vanity address grinding, agent passport, tx decode, Pump.fun token and creator intelligence, AI revenue analysis, and 12 MCP tools for avatar and model management."
use_case: "Use for Solana agent identity and reputation, Pump.fun token and creator intel, glTF/GLB model validation and optimization, rendering a Solana token as a GLB, grinding a vanity Solana address, decoding Solana transactions, and AI revenue analysis."
category: ai_ml
version: "1.0.0"
...
Agent-first 3D and Solana identity platform. Eight paid REST endpoints plus one MCP endpoint ($0.001/call, Solana USDC via x402/MPP) covering 12 tools.
...
### `POST /api/mcp` — $0.001 USDC/call (MCP Streamable HTTP, Solana USDC via x402/MPP)
...
- `GET /api/explore` — paginated directory of onchain agents + public avatars (q, chain, only3d, source, cursor)
- `GET /api/showcase` — agents with a 3D avatar on three.ws
- `GET /api/avatars/public` — keyword-searchable public avatar browse
- `GET /api/agents/{id}/manifest` — canonical agent discovery card
- `GET /api/agents/by-wallet?address=` — agents registered to a wallet address
- `GET /api/pump/curve?mint=` — Pump.fun bonding-curve snapshot (spot price, market cap, graduation progress)
- `GET /api/pump/quote-sdk?mint=&side=&amount=` — deterministic buy/sell quote with price impact
- `GET /api/pump/channel-feed` — live pump.fun feed: mints, whales, social-fee claims
...
- Use `GET /api/x402/pumpfun-creator-intel` before `pumpfun-token-intel` when you only need creator trust signals — cheaper and faster.
